...
class SearchScreen extends React.Component {
  ...
  doSearchUser = () => {
    ...
    //query firebase db for values both uppercase and lowercase that match the input
      ...
      .then((ref) => {
        ref.forEach((user) => {
          if (user.key != this.state.currentUser.toLowerCase()) {
            //Push found user info to user flatlist
            tempFound.push({
              username: user.key,
              uid: user.val().id
            })
  ...
  doSearchGroup = () => {
    ...
    //query groups based on input
    ref.orderByKey().startAt(searchInput).endAt(searchInput + "\uf8ff").once("value")
      .then((ref) => {
        ref.forEach((group) => {
          //push found groups to groups flatlist
          tempFound.push({
            name: group.key,
